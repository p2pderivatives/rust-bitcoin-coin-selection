...
use bitcoin_units::{Amount, CheckedSum, FeeRate, SignedAmount, Weight};
...
/// weight is exceeded, then the least valuable inputs are removed.  In so doing, minimize the
/// number of `UTXOs` included in the result.
///
/// # Parameters
///
...
///   use a weight of 10,000 vB (40,000 WU) for this parameter.
...
pub fn select_coins_srd<'a, R: rand::Rng + ?Sized, Utxo: WeightedUtxo + std::cmp::Ord>(
    ...
    let mut heap: BinaryHeap<Reverse<&Utxo>> = BinaryHeap::new();
...
mod tests {
    ...
    fn select_coins_skip_negative_effective_value() {
        // After randomization, the effective values are: [-1 sats, 2 cBTC]
        // The first UTXO is skipped and then 2 cBTC fulfills the target.
        TestSRD {
            target: "1.9 cBTC",
            fee_rate: "10 sat/kwu",
            max_weight: "40000 wu",
            weighted_utxos: &["e(2 cBTC)/68 vB", "e(-1 sat)/68 vB"],
    ...
    fn select_coins_srd_max_weight_without_error() {
        ...
        for _i in 0..60 {
            weighted_utxos.push("33 cBTC/272 wu");
        }
        for _i in 0..10 {
            weighted_utxos.push("2 BTC/272 wu");
        }

        for _i in 0..9 {
            expected_utxos.push("2 BTC/272 wu");
        }
        for _i in 0..27 {
            expected_utxos.push("33 cBTC/272 wu");
        }
        ...
        TestSRD {
            ...
            fee_rate: "0",
            max_weight: "10000 wu",
            weighted_utxos: &weighted_utxos,
            expected_utxos: &expected_utxos,
            expected_error: None,
            expected_iterations: 68,
        }
        .assert();
    }
    ...
    fn select_coins_srd_proptest() {
        arbtest(|u| {
            ...
            let result: Result<_, _> =
                select_coins_srd(target, fee_rate, max_weight, &utxos, &mut get_rng());
