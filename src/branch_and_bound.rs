...
mod tests {
    ...
    fn select_coins_bnb_solution_proptest() {
        arbtest(|u| {
            ...
            let mut target_inputs = crate::tests::Pool::arbitrary(u)?;
            let mut filler_inputs = crate::tests::Pool::arbitrary(u)?;
            target_inputs = target_inputs.set_rates(fee_rate, long_term_fee_rate);
            filler_inputs = filler_inputs.set_rates(fee_rate, long_term_fee_rate);
            
            let target = effective_sum(&target_inputs.utxos).unwrap_or(Amount::ZERO);

            let mut pool = vec![];
            pool.append(&mut target_inputs.utxos.clone());
            pool.append(&mut filler_inputs.utxos.clone());
            assert!(pool.len() == target_inputs.utxos.len() + filler_inputs.utxos.len());
            let result = branch_and_bound(target, cost_of_change, max_weight, &pool);
            ...
            match result {
                ...
                Err(InsufficentFunds) => {
                    assert!(effective_sum(&target_inputs.utxos).unwrap() < target);
                }
                Err(IterationLimitReached) => {}
                Err(Overflow(_)) => {
                    assert!(
                        effective_sum(&pool).is_none()
                            || weight_sum(&pool).is_none()
                            || (target + cost_of_change).is_error()
                    );
                }
                Err(ProgramError) => panic!("un-expected result"),
                Err(SolutionNotFound) => {
                    assert!(target_inputs.utxos.is_empty() || target == Amount::ZERO)
                }
                Err(MaxWeightExceeded) => {
                    assert!(weight_sum(&pool).unwrap() > max_weight);
                }
            }
    ...
    fn select_coins_bnb_thrifty_proptest() {
        arbtest(|u| {
            ...
            let result_a = branch_and_bound(target, cost_of_change, max_weight, &pool.utxos);

            let fee_rate_a = pool.fee_rate;
            let fee_rate_b = pool.long_term_fee_rate;
            // swap lt_fee_rate and fee_rate position.
            pool.set_rates(fee_rate_b, fee_rate_a);
            let result_b = branch_and_bound(target, cost_of_change, max_weight, &pool.utxos);
