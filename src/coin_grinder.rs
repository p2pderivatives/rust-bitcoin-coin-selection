...
pub fn select_coins(
    ...
    weighted_utxos
        .sort_by(|a, b| b.effective_value.cmp(&a.effective_value).then(b.weight.cmp(&a.weight)));
...
mod tests {
    ...
    impl TestCoinGrinder<'_> {
        fn assert(&self) {
            ...
            let result = select_coins(target, change_target, max_weight, &pool.utxos);
    ...
    fn min_tail_weight() {
        ...
        let pool: UtxoPool = UtxoPool::new(weighted_utxos, FeeRate::ZERO, FeeRate::MAX);
