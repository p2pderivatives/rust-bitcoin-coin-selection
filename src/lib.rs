...
pub struct WeightedUtxo {
    ...
    weight: Weight,
    /// Calculated positive effective value `(value - fee)`.  The value is stored as a `u64` for
    /// better performance.
    effective_value: u64,
    /// Current `SignedAmount` required to spend the output at the given `fee_rate`.
    fee: SignedAmount,
...
impl WeightedUtxo {
    ...
    pub fn new(
        ...
        if let Some(effective_value) = positive_effective_value {
            ...
            let waste_score = Self::calculate_waste_score(fee, long_term_fee);
            return Some(Self { value, weight, effective_value, fee, long_term_fee, waste_score });
        }
    ...
    fn effective_value(&self) -> Amount { Amount::from_sat(self.effective_value).unwrap() }
