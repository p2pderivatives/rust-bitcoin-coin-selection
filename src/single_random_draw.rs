...
mod tests {
    ...
    fn select_coins_skip_negative_effective_value() {
        ...
        TestSRD {
            ...
            fee_rate: "10 sat/kwu",
            weighted_utxos: &["1 cBTC", "2 cBTC", "e(-1 sat)"],
    ...
    fn select_coins_srd_change_output_too_small() {
        ...
        TestSRD {
            target: "3 cBTC",
            fee_rate: "10 sat/kwu",
            weighted_utxos: &["e(1 cBTC)", "e(2 cBTC)"],
            expected_utxos: None,
            expected_iterations: 0,
        }
        .assert();
    ...
    fn select_coins_srd_with_high_fee() {
        ...
        TestSRD {
            target: "2 cBTC",
            fee_rate: "10 sat/kwu",
            weighted_utxos: &["1 cBTC", "2050000 sats"],
            expected_utxos: Some(&["2050000 sats", "1 cBTC"]),
            expected_iterations: 2,
        }
        .assert();
    }

    // This can be removed with the limit of InputWeightPrediction size
    //#[test]
