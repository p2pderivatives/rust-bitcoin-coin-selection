...
mod tests {
    ...
    fn select_coins_bnb_solution_proptest() {
        arbtest(|u| {
            ...
            let mut solutions = UtxoPool::arbitrary(u)?;

            let cost_of_change = Amount::arbitrary(u)?;
            let fee_rate_a = FeeRate::arbitrary(u)?;
            let fee_rate_b = FeeRate::arbitrary(u)?;

            let known_target: Option<SignedAmount> =
                solutions.utxos.iter().try_fold(SignedAmount::ZERO, |acc, u| {
                    let e = u.effective_value(fee_rate_a).unwrap_or(SignedAmount::ZERO);
                    acc.checked_add(e)
                });

            let mut target = Amount::ZERO;
            if let Some(t) = known_target {
                if t.is_positive() {
                    target = t.to_unsigned().unwrap();
                }
            }

            let upper_bound = target.checked_add(cost_of_change).unwrap_or(Amount::ZERO);
            let new_target = u.int_in_range::<u64>(target.to_sat()..=upper_bound.to_sat()).unwrap();
            let new_target = Amount::from_sat(new_target);

            pool.utxos.append(&mut solutions.utxos);
            pool.utxos.shuffle(&mut thread_rng());

            let result_a =
                select_coins_bnb(new_target, cost_of_change, fee_rate_a, fee_rate_b, &pool.utxos);

            let result_b =
                select_coins_bnb(new_target, cost_of_change, fee_rate_b, fee_rate_a, &pool.utxos);
            ...
            if let Some((i, utxos_a)) = result_a {
                ...
                let sum = utxos_a
                    .iter()
                    .map(|u| u.effective_value(fee_rate_a).unwrap_or(SignedAmount::ZERO))
                    .checked_sum()
                    .unwrap()
                    .to_unsigned()
                    .unwrap();
                ...
                let sum = pool.available_value(fee_rate_a);
