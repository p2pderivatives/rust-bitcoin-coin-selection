...
pub struct WeightedUtxo {
    /// The max `Amount` contributed towards a target `Amount`.
    value: Amount,
    /// An estimated `Weight` `(satisfaction weight + base weight)`.
    weight: Weight,
    ...
    /// A score which represents how wasteful it is to spend this `WeightedUtxo` give the current
    /// fee environment.
    waste_score: i64,
}
